# Question given by Howard Grimberg of Amazon Web Services
# Goal Time of Completion: 10 min
...
# Complicated Unnecessary solution
def calcGas(array):
    curGas = 0
    minGas = 0
    for index in range(1, len(array)):
        curHeight = array[index]
        diff = array[index - 1] - curHeight
        curGas += diff
        minGas = min(minGas, curGas)
    return abs(minGas)

# Faster Solution
# It turns out all you need is the difference between the starting height and the max height
def calcGas2(array):
    start = array[0][z]
    maxHeight = 0
    for index in range(1, len(array)):
        maxHeight = max(array[index], maxHeight)
    return maxHeight - start
